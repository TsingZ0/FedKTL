...
- `stable-diffusion/`: 
    ...
    - `v1.5/`: The folder to store the pre-trained Stable Diffusion v1.5. We only reserved the sub-folder names here due to limited space. Please download the entire model from the [Hugging Face link](https://huggingface.co/runwayml/stable-diffusion-v1-5/tree/main). Regarding large model files, only the primary `safetensors` files are necessary within each sub-folder. For further instructions on running Stable Diffusion, please consult the documentation of the `diffuser` package. 
- `stylegan/`: 
